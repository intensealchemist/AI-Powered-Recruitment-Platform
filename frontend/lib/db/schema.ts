...
import { type Experience, type Skill, type Project, type Education, type AIConversationLog } from "../types";
...
export const profiles = sqliteTable("profiles", {
  ...
  experiences: text("experiences", { mode: "json" }).$type<Experience[]>().default([]),
  skills: text("skills", { mode: "json" }).$type<Skill[]>().default([]),
  projects: text("projects", { mode: "json" }).$type<Project[]>().default([]),
  education: text("education", { mode: "json" }).$type<Education[]>().default([]),
  conversationLogs: text("conversation_logs", { mode: "json" }).$type<AIConversationLog[]>().default([]),
